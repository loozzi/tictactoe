...
// function get computer turn and play
function playComputerTurn() {
  ...
  if (cx !== -2 && cy !== -2) {
    ...
    setText(cx, cy, turn);
    if (isFinalState(data)) showWinner(data);
  }
}
...
function getNextPlayer(data) {
  ...
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      if (dataInput[i][j] === 1) player1++;
      else if (dataInput[i][j] === 2) player2++;
    }
  }
...
function isFinalState(data) {
  ...
  for (let i = 0; i < 3; i++)
    for (let j = 0; j < 3; j++) if (dataInput[i][j] == 0) return false;
...
const playerChoose = prompt("Choose player? X or O");
if (playerChoose === "O" || playerChoose === "o") {
  playComputerTurn();
}
